...
#if (CapSense_CSD_CURRENT_SOURCE == CapSense_CSD_EXTERNAL_RB)
    uint8  CapSense_CSD_RbCh0_cur = CapSense_CSD_RBLEED1;
    #if (CapSense_CSD_DESIGN_TYPE == CapSense_CSD_TWO_CHANNELS_DESIGN)
        uint8  CapSense_CSD_RbCh1_cur = (CapSense_CSD_RBLEED1 + CapSense_CSD_TOTAL_RB_NUMBER__CH0);
    #endif /* (CapSense_CSD_DESIGN_TYPE == CapSense_CSD_TWO_CHANNELS_DESIGN)*/ 
#else
    #if (CY_PSOC5A)
        uint8 CapSense_CSD_idac_cfg_restore = 0u;
        uint8 CapSense_CSD_idac_ch0_cr0reg = 0u;
        #if (CapSense_CSD_DESIGN_TYPE == CapSense_CSD_TWO_CHANNELS_DESIGN)
            uint8 CapSense_CSD_idac_ch1_cr0reg = 0u;
        #endif /* (CapSense_CSD_DESIGN_TYPE == CapSense_CSD_TWO_CHANNELS_DESIGN) */ 
    #endif /* (CY_PSOC5A) */
...
const uint8 CYCODE CapSense_CSD_idacSettings[] = {
    100u,100u,150u,150u,150u,150u,150u,
};

const uint8 CYCODE CapSense_CSD_widgetResolution[] = {
    CapSense_CSD_PWM_RESOLUTION_10_BITS,
    CapSense_CSD_PWM_RESOLUTION_10_BITS,
    CapSense_CSD_PWM_RESOLUTION_10_BITS,
};

uint8 CapSense_CSD_AnalogSwitchDivider = 7u;
...
void CapSense_CSD_Init(void) 
{
    ...
    CyIntSetVector(CapSense_CSD_IsrCH0_ISR_NUMBER, CapSense_CSD_IsrCH0_ISR);
...
void CapSense_CSD_PreScan(uint8 sensor) CYREENTRANT
{
    ...
    CapSense_CSD_CONTROL_REG |= CapSense_CSD_CTRL_SYNC_EN;
            ...
            *CapSense_CSD_rbTable[CapSense_CSD_RbCh0_cur] |= CapSense_CSD_BYP_MASK;
        ...
        if((CapSense_CSD_CONTROL_REG & CapSense_CSD_CTRL_WINDOW_EN__CH1) != 0u)
        {
            ...
            #else
                /* Connect DSI output to Rb */
                *CapSense_CSD_rbTable[CapSense_CSD_RbCh1_cur] |= CapSense_CSD_BYP_MASK;
    ...
    ********************************************************************************/
    void CapSense_CSD_InitRb(void) 
    {
        ...
        /* Disable all Rb */
        for(i=0; i < CapSense_CSD_TOTAL_RB_NUMBER; i++)
        {
            /* Make High-Z */
    ...
    *
    * Global Variables:
    *  CapSense_CSD_RbCh0_cur - used to store current number of active 
    *  bleed resistor (Rb) of channel 0.
    *  CapSense_CSD_RbCh1_cur - used to store current number of active 
    ...
    *******************************************************************************/
    void CapSense_CSD_SetRBleed(uint8 rbleed) 
    {
            ...
            {
                CapSense_CSD_RbCh1_cur = (rbleed - CapSense_CSD_TOTAL_RB_NUMBER__CH0);   
            }
    ...
    *******************************************************************************/
    void CapSense_CSD_SetPrescaler(uint8 prescaler) CYREENTRANT
    {
        /* Set Prescaler */
            ...
            CapSense_CSD_PRESCALER_COMPARE_REG = (prescaler >> 0x01u);
        #elif (CapSense_CSD_PRESCALER_OPTIONS == CapSense_CSD_PRESCALER_FF)
            CY_SET_REG16(CapSense_CSD_PRESCALER_PERIOD_PTR, (uint16) prescaler);
            CY_SET_REG16(CapSense_CSD_PRESCALER_COMPARE_PTR, (uint16) (prescaler >> 0x01u));
        #else
            /* Do nothing = config without prescaler */
        #endif  /* (CapSense_CSD_PRESCALER_OPTIONS == CapSense_CSD_PRESCALER_UDB) */
    }
